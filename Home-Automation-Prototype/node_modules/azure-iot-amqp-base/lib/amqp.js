...
var Amqp = (function () {
    ...
    Amqp.prototype.connect = function (uri, sslOptions, done) {
        ...
        if (!this._connected) {
            ...
            var connectErrorHandler_1 = function (err) {
                connectError_1 = err;
            };
            var amqpErrorHandler_1 = function (err) {
                debug('amqp10 error: ' + err.toString());
                if (_this._disconnectHandler) {
                    _this._disconnectHandler(err);
                }
            };
            this._amqp.on(_amqpClientError, connectErrorHandler_1);
            this._amqp.connect(this.uri)
                .then(function (result) {
                debug('AMQP transport connected.');
                _this._connected = true;
                _this._amqp.on(_amqpClientError, amqpErrorHandler_1);
                _this._amqp.removeListener(_amqpClientError, connectErrorHandler_1);
    ...
    Amqp.prototype.setDisconnectHandler = function (disconnectCallback) {
        ...
        this._amqp.on('connection:closed', function () {
            _this._connected = false;
            _this._disconnectHandler(new Error('amqp10: connection closed'));
        });
    };
    /**
    ...
    Amqp.prototype.disconnect = function (done) {
        ...
        Promise.all(openLinks.map(function (link) { return link.detach(); }))
            .then(function () { return _this._amqp.disconnect(); })
            .then(function (result) {
    ...
    Amqp.prototype.attachReceiverLink = function (endpoint, linkOptions, done) {
        ...
        if (!this._connected) {
            ...
            self_1._amqp.createReceiver(endpoint, linkOptions)
                ...
                receiver.on('detached', function (detached) {
                    debug('receiver link detached: ' + endpoint);
                    if (self_1._receivers[endpoint]) {
                        delete self_1._receivers[endpoint];
                    }
                });
        ...
        if (!this._connected) {
            ...
            self_2._amqp.createSender(endpoint, linkOptions)
                ...
                sender.on('detached', function () {
                    ...
                    if (self_2._senders[endpoint]) {
                        delete self_2._senders[endpoint];
                    }
                });
